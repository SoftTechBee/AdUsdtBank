...
public partial class User_Default : System.Web.UI.Page
{
    ...
    public void loaddirect()
    {
        try
        {
            ...
            if (username != "Admin")
            {
                sql = " select *,'A'+cast(packid as nvarchar(10)) as pack from register where reffid ='" + SessionData.Get<string>("newuser") + "'";

            }
            else
            {

                sql = " select packid,username, name, mobile, city, reffname, reffid,status, dateofjoining,joinamount from register";
