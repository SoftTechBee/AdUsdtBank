...
public class clsDashboard
{
    ...
    public string IncomeTypeHold(string username)
    {
        string total = "0";
        try
        {
            if (username == "Admin")
            {
                string sql = "select sum(CONVERT(decimal(18,2),ISNULL(credit,0))) as amount from accountreserv where   status='0'";
                DataTable dt = objcon.ReturnDataTableSql(sql);
                if (dt.Rows.Count > 0)
                {
                    total = dt.Rows[0][0].ToString();
            ...
            {
                string sql = "select sum(CONVERT(decimal(18,2),ISNULL(credit,0))) as amount from accountreserv where username='" + username + "' and   status='0'";
                DataTable dt = objcon.ReturnDataTableSql(sql);
                if (dt.Rows.Count > 0)
                {
                    total = dt.Rows[0][0].ToString();
    ...
    public string IncomeTypeHoldDebit(string username)
    {
        string total = "0";
        try
        {
            if (username == "Admin")
            {
                string sql = "select sum(CONVERT(decimal(18,2),ISNULL(debit,0))) as amount from accountreserv where   status='0'";
                DataTable dt = objcon.ReturnDataTableSql(sql);
                if (dt.Rows.Count > 0)
                {
                    total = dt.Rows[0][0].ToString();
            ...
            {
                string sql = "select sum(CONVERT(decimal(18,2),ISNULL(debit,0))) as amount from accountreserv where username='" + username + "' and   status='0'";
                DataTable dt = objcon.ReturnDataTableSql(sql);
                if (dt.Rows.Count > 0)
                {
                    total = dt.Rows[0][0].ToString();
    ...
    public string PoolTeam(string username,string Side)
    {
        string total = "0";
        try
        {
                ...
                string sql = "EXEC [dbo].[PoolTeam]@Sponser = '" + username + "' ,@Side ='"+ Side + "'";
                DataTable dt = objcon.ReturnDataTableSql(sql);
                if (dt.Rows.Count > 0)
                {
                    total = dt.Rows[0][0].ToString();
    ...
    public string IncomeTypeAvlHold(string username)
    {
        string total = "0";
        try
        {
            if (username == "Admin")
            {
                string sql = "select (sum(CONVERT(decimal(18,2),ISNULL(credit,0)))-sum(CONVERT(decimal(18,2),ISNULL(debit,0)))) as amount from accountreserv ";
                DataTable dt = objcon.ReturnDataTableSql(sql);
                if (dt.Rows.Count > 0)
                {
                    total = dt.Rows[0][0].ToString();
            ...
            {
                string sql = "select  (sum(CONVERT(decimal(18,2),ISNULL(credit,0)))-sum(CONVERT(decimal(18,2),ISNULL(debit,0)))) as amount from accountreserv where username='" + username + "' and status='0'";
                DataTable dt = objcon.ReturnDataTableSql(sql);
                if (dt.Rows.Count > 0)
                {
                    total = dt.Rows[0][0].ToString();
  ...
  public string DepositDebit(string username,string type)
    ...
    public string ReturnWallet(string username)
    {
        string TotalTeamBusness = "0";
        try
        {
            clsConnection objcon1 = new clsConnection();
            string sql = "select WalletAddress from register where username='" + username + "'";
            DataTable dt = objcon1.ReturnDataTableSql(sql);
            if (dt.Rows.Count > 0)
            {
                TotalTeamBusness = dt.Rows[0][0].ToString();
    ...
    public string ReturnCurPool(string username)
    {
        ...
        {
            clsConnection objcon1 = new clsConnection();
            string sql = "select max(PoolNo) as LastSlot from TblTransaction where username='" + username + "'";
            DataTable dt = objcon1.ReturnDataTableSql(sql);
            if (dt.Rows.Count > 0)
            {
